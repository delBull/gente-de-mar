...
export function ConnectButton(props) {
    ...
    const autoConnectComp = props.autoConnect !== false && (_jsx(AutoConnect, { chain: preferredChain, appMetadata: props.appMetadata, client: props.client, wallets: wallets, timeout: typeof props.autoConnect === "boolean"
            ? undefined
    ...
    if (!localeQuery.data) {
        ...
        return (_jsxs(AnimatedButton, { disabled: true, className: combinedClassName, variant: "primary", type: "button", style: {
                minWidth: "165px",
                height: "50px",
                ...props.connectButton?.style,
            }, children: [autoConnectComp, _jsx(Spinner, { size: "sm", color: "primaryButtonText" })] }));
    }
    return (_jsxs(WalletUIStatesProvider, { theme: props.theme, isOpen: false, children: [_jsx(ConnectButtonInner, { ...props, siweAuth: siweAuth, connectLocale: localeQuery.data }), _jsx(ConnectModal, { shouldSetActive: true, accountAbstraction: props.accountAbstraction, auth: props.auth, chain: preferredChain, chains: props.chains, client: props.client, connectLocale: localeQuery.data, meta: {
...
function ConnectButtonInner(props) {
    ...
    if (!activeAccount) {
        ...
        const combinedClassName = `${props.connectButton?.className || ""} ${TW_CONNECT_WALLET}`;
        return (_jsx(AnimatedButton, { disabled: isLoading, className: combinedClassName, "data-theme": theme, "data-is-loading": isLoading, variant: "primary", type: "button", style: {
                minWidth: "165px",
                height: "50px",
                fontSize: "16px",
                ...props.connectButton?.style,
    ...
    if (siweAuth.requiresAuth) {
        ...
        if (siweAuth.isPending || siweAuth.isLoggingIn || siweAuth.isLoggingOut) {
            ...
            return (_jsx(AnimatedButton, { disabled: true, className: combinedClassName, variant: "primary", type: "button", style: {
                    ...
                    height: "50px",
                    ...props.connectButton?.style,
                }, children: _jsx(Spinner, { size: "sm", color: "primaryButtonText" }) }));
        }
        // sign in button + modal if *not* loading and *not* logged in
        if (!siweAuth.isLoggedIn) {
            return (_jsxs(_Fragment, { children: [_jsx(Button, { variant: "primary", type: "button", onClick: () => {
                            setShowSignatureModal(true);
                        }, className: props.signInButton?.className, style: {
                            minWidth: "165px",
                            minHeight: "50px",
                            ...props.signInButton?.style,
                        }, children: siweAuth.isLoggingIn ? (_jsx(Spinner, { size: "sm", color: "primaryButtonText" })) : (_jsxs(Container, { flex: "row", center: "y", gap: "sm", children: [_jsx(LockIcon, { size: iconSize.sm }), _jsxs("span", { children: [" ", props.signInButton?.label || locale.signIn, " "] })] })) }), _jsx(Modal, { size: "compact", open: showSignatureModal, setOpen: setShowSignatureModal, children: _jsx(SignatureScreen, { client: props.client, connectLocale: locale, modalSize: "compact", termsOfServiceUrl: props.connectModal?.termsOfServiceUrl, privacyPolicyUrl: props.connectModal?.privacyPolicyUrl, onDone: () => setShowSignatureModal(false), auth: props.auth }) })] }));
        }
        // otherwise, show the details button
    }
    return (_jsx(AccountProvider, { address: activeAccount.address, client: props.client, children: _jsx(ConnectedWalletDetails, { theme: theme, detailsButton: props.detailsButton, detailsModal: {
                ...props.detailsModal,
