...
function useSendTransaction(config = {}) {
    ...
    const showPayModal = (data) => {
        if (payModal === false)
            return;
        setRootEl((0, jsx_runtime_1.jsx)(TransactionModal_js_1.TransactionModal, { title: payModal?.metadata?.name || "Transaction", txId: (0, random_js_1.randomBytesHex)(), tx: data.tx, onComplete: data.sendTx, onClose: () => {
                setRootEl(null);
                data.rejectTx(new Error("User rejected transaction by closing modal"));
            }, onTxSent: data.resolveTx, client: data.tx.client, localeId: payModal?.locale || "en_US", supportedTokens: payModal?.supportedTokens, theme: payModal?.theme || "dark", modalMode: data.mode, payOptions: {
